...
modified_flow[1:4] = Supply_Input[1:4]
names(modified_flow) = c(names(Supply_Input)[1:4], mod_flow_list)
...
if (simulate_demand == 1) {
	...
	interruptible_demand <- data.frame(matrix(nrow=N, ncol=length(new_pod_names)+6, 0))
}
...
#natflow_list = c("AM", "IP", "PAL", "MIN", "LUC", "Minidoka", "LowerBoise", "Henry", "LUC")
...
names_Output = c("Week", "Month", "Day", "Year", "BRRunoffAprJul", "DARunoffAprAug", "DARunoffAprSep", "DARunoffJanJul", "DURunoffAprAug", "DWRunoffAprJul", "HHRunoffAprAug", "HHRunoffMaySep",
	...
	paste0("Iflow", stn_colsim[match(mainstem_names, stn_colsim[,1]),2]))
...
weekly_AssuredRefill = read.table("~/Step_4/AssuredRefill_new.txt", header=T)
# 2- Weekly to each time step
# this part applies the weekly assured refill curves (52 week/year) to all of the years
ncols = ncol(weekly_AssuredRefill) + 2
AssuredRefill_EntireTime = data.frame(matrix(ncol=ncols, nrow=N, 0))
names(AssuredRefill_EntireTime) = c("WeekInYear", "Month","Year", dam_lookup$name1[match(names(weekly_AssuredRefill)[-1], dam_lookup$name2)])
AssuredRefill_EntireTime[,1:3] = Output_to_ColSim[1:N,c(1,2,4)]
...
for(i_year in 1:Number_of_Years){ 
	...
	if (L==52) {
		AssuredRefill_EntireTime[ind1:ind2,4:ncols] = weekly_AssuredRefill[,2:(ncols-2)]
	} else {
		AssuredRefill_EntireTime[ind1:(ind2-1),4:ncols] = weekly_AssuredRefill[1:(L-1),2:(ncols-2)]
		AssuredRefill_EntireTime[ind2,4:ncols] = weekly_AssuredRefill[L-1,2:(ncols-2)]
	}
}

tmp = aggregate(modified_flow$DALLE, list(modified_flow$Year, modified_flow$Month<=7), sum)
DallesRunoffJanJul = tmp[tmp[,2]==1 & tmp[,1]>begin_year,3]
DA_forecast = data.frame(unique(AssuredRefill_EntireTime$Year)[-1], DallesRunoffJanJul)
...
PDR_lower <- read.table("~/Step_4/AssumedRelease_min.txt", header=T)[-1]
