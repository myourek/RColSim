...
GCCurtail <- function() {
	if (all_curtailment == 1) {
		GCCurtail_0 <- max(0, DemVICGC + GCEvap() + BDDem() - AFPrelim_c - ARPrelim() - CLPrelim_c - (GrandCouleeFlowData() - AlbeniFallFlowData() - ArrowFlowData() - CorraLinnFlowData()))
...
GCInflow <- function() {
	if (ResetStorage() == 1) {
		...
		GCInflow_o <- GCInc() + Canada_Outflows() + BDOut() - GCDem() - GCEvap()
	}
	return(GCInflow_o)
}
...
CJCurtail <- function() {
	if (simulate_curtailment == 1) {
		CJCurtail_0 <-  min(DemVICCJ, max(IflowCJ + DemVICCJ - GCPrelim_c - CJInc(), 0))
...
RICurtail <- function() {
	if (simulate_curtailment == 1) {
		RICurtail_0 <- min(DemVICRI, max(IflowRI + DemVICRI - RRPrelim() - RIInc(), 0))
...
RRCurtail <- function() {
	if (simulate_curtailment == 1) {
		RRCurtail_0 <- min(DemVICRR, max(IflowRR + DemVICRR - WEPrelim() - RRInc(), 0))
...
RRPrelim <- function() {
	UpDemand <- RRDem() + WEDem_c + CJDem_c
...
WACurtail <- function() {
	if (simulate_curtailment == 1) {
		WACurtail_0 <- min(DemVICWA, max(IflowWA + DemVICWA - RIPrelim_c - WAInc(), 0))
...
WAPrelim <- function() {
	UpDemand <- CJDem_c + WEDem_c + RRDem_c + RIDem_c + WADem()
...
WECurtail <- function() {
	if (simulate_curtailment == 1) {
		WECurtail_0 <- min(DemVICWE, max(IflowWE + DemVICWE - CJPrelim_c - WEInc(), 0))
...
PRCurtail <- function()	{
	if (simulate_curtailment == 1) {
		PRCurtail_0 <- min(DemVICPR, max(IflowPR + DemVICPR - WAPrelim_c - PRInc(), 0))
...
PRPrelim <- function() {
	...
	UpDemand <- CJDem_c + WEDem_c + RRDem_c + RIDem_c + WADem_c + PRDem()
...
JDCurtail <- function() {
	if (simulate_curtailment == 1) {
		JDCurtail_0 <- min(DemVICJD, max(IflowJD + DemVICJD - MCNPrelim_c - JDInc(), 0))
...
DACurtail <- function() {
	if (simulate_curtailment == 1) {
		DACurtail_0 <- min(DemVICDA, max(IflowDA + DemVICDA - JDPrelim_c - DAInc(), 0))
...
AFCurtail <- function() {
	if (all_curtailment == 1) {
		AFCurtail_0 <- max(DemVICAF + AFEvap() - AFInc() - CBPrelim(), 0)
...
BONCurtail <- function() {
	if (all_curtailment == 1) {
		BONCurtail_0 <- max(DemVICBON - DAPrelim_c - BONInc(), 0)
...
COLCurtail <- function() {
	if (all_curtailment == 1) {
		COLCurtail_0 <- max(DemVICCOL - HHPrelim_c - COLInc(), 0)
...
COLDem <- function() {
	COLDem_o <- DemVICCOL - COLCurtail()
	return(COLDem_o)
}
ColumbiaFalls <- function() {
	ColumbiaFalls_o <- max(0, HHPrelim_c + COLInc() - COLDem())
...
LGCurtail <- function() {
	if (all_curtailment == 1) {
		LGCurtail_0 <- max(DemVICLG - DWPrelim_c - HCOut() - LGInc(), 0)
...
LIGCurtail <- function() {
	if (all_curtailment == 1) {
		LIGCurtail_0 <- max(DemVICLIG - LGPrelim_c - LIGInc(), 0)
...
LIGDem <- function() {
	LIGDem_o <- DemVICLIG - LIGCurtail()
	LIGDem_c <<- LIGDem_o
	return(LIGDem_o)
}
LIGPrelim <- function() {
	UpDemand <- OXDem() + HCDem() + LGDem_c + LIGDem()
...
LMCurtail <- function() {
	if (all_curtailment == 1) {
		LMCurtail_0 <- max(DemVICLM - LIGPrelim_c - LMInc(), 0)
...
LMPrelim <- function() {
	UpDemand <- OXDem() + HCDem() + LGDem_c + LIGDem_c + LMDem()
...
IHCurtail <- function() {
	if (all_curtailment == 1) {
		IHCurtail_0 <- max(DemVICIH - LMPrelim_c - IHInc(), 0)
...
CLCurtail = function() {
	if (simulate_curtailment == 1) {
		CLCurtail_0 <- max(CLEvap() + DemVICBONF + DemVICCL - DUPrelim_c - LBPrelim_c - (CorraLinnFlowData() - DuncanFlowData() - LibbyFlowData()), 0)
...
CLInflow <- function() {
	if (ResetStorage() == 1) {
		...
		CLInflow_o <- DUOutflow_c + BonnersFerry() + CLInc() - CLDem() - CLEvap()
	}
...
BRCurtail <- function() {
	if (simulate_curtailment == 1) {
		BRCurtail_0 <- max(0, DemVICBR + BREvap() - MSPrelim_c - BRInc())
...
MCNCurtail <- function() {
	if (simulate_curtailment == 1) {
		MCNCurtail_0 <- min(DemVICMCN, max(IflowMCN + DemVICMCN - IHPrelim() - PRPrelim() - MCNInc(), 0))
...
MCNPrelim <- function() {
	if (I_Week == 1) {
		MCNPrelim_o <- LowerPrelimUpFlow_c + McNaryFlowData()
	} else {
		MCNPrelim_o <- max(0, IHPrelim_c + PRPrelim_c + MCNInc() - MCNDem())
	}
	MCNPrelim_c <<- MCNPrelim_o
	return(MCNPrelim_o)
}
MCNetHead <- function() {
	MCNetHead_o <- 74
...
MILCurtail <- function() {
	if (simulate_curtailment == 1) {
		MILCurtail_0 <- max((DemVICPAL + DemVICMIL + MSEvap()) - (USPrelim() + MSInc()), 0)
...
BONFCurtail <- function() {
	if (all_curtailment == 1) {
		BONFCurtail_0 <- max(DemVICBONF - LBPrelim() - BONFInc(), 0)
...
BONFDem <- function() {
	BONFDem_o <- DemVICBONF - BONFCurtail()
	return(BONFDem_o)
}
BONFInc <- function() {
...
CBCurtail <- function() {
	if (all_curtailment == 1) {
		CBCurtail_0 <- max(DemVICCB - NOXPrelim() - CBInc(), 0)
...
BDCurtail <- function() {
	if (all_curtailment == 1) {
		BDCurtail_0 <- max(0, DemVICBD - AFPrelim() - BDInc())
...
BDPrelim <- function() {
	BDPrelim_o <- AFPrelim() + BDInc() - BDDem()
	return(BDPrelim_o)
}
...
OXCurtail <- function() {
	if (all_curtailment == 1) {
		OXCurtail_0 <- max(DemVICOX - BRPrelim() - OXInc(), 0)
...
OXDem <- function() {
	OXDem_o <- DemVICOX - OXCurtail()
	OXDem_c <<- OXDem_o
	return(OXDem_o)
}
OXIn <- function() {
	OXIn_o <- max(0, BRPrelim() + OXInc() - OXDem())
	return(OXIn_o)
}
OXOut <- function() {
	OXOut_o <- OXIn()
