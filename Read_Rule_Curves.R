...
Read_Rule_Curves <- function() {
	...
	DUFlood_input <<- read.table(paste(input_dir, "DUECCandFC/DUFlood.txt", sep=""), header=T)
	DU_JAN_FAMAJ <<- read.table(paste(input_dir, "DUECCandFC/DU-JAN-FAMAJ.txt", sep=""), header=T)
	ARFlood <<- read.table(paste(input_dir, "ARECCandFC/ARFlood.txt", sep=""), header=T)
	...
	DWFlood_input <<- read.table(paste(input_dir, "/DW/DWFlood.txt", sep=""), header=T)
	...
	DWCriticalCurve_input <<- read.table(paste(input_dir, "/DW/DWCriticalCurve.txt", sep=""), header=T)
	...
	BRFlood <<- read.table(paste(input_dir, "/BRL/BRFlood.txt", sep=""), header=T)
	...
	LBFlood_input <<- read.table(paste(input_dir, "/Libby/LBFlood.txt", sep=""), header=T)
	...
	LBCriticalCurve_input <<- read.table(paste(input_dir, "/Libby/LBCriticalCurve.txt", sep=""), header=T)
	...
	CLIJCRuleCurve_input <<- read.table(paste(input_dir, "/CL/CLIJCRuleCurve.txt", sep=""), header=T)
  
	##### Columbia Falls ######
	ColFall_Target <<- read.table(paste(input_dir, "/HH/ColFalls_Target.txt", sep=""), header=T) 
  
	##### Hungary Horse ########
	HHCriticalCurve_input <<- read.table(paste(input_dir, "/HH/HHCriticalCurve.txt", sep=""), header=T)
	HHBaseDraftLimit_input <<- read.table(paste(input_dir, "/HH/HHBaseDraftLimit.txt", sep=""), header=T)
	HHAssuredRefill_input <<- read.table(paste(input_dir, "/HH/HHAssuredRefill.txt", sep=""), header=T)
	HHStor <<- read.table(paste(input_dir, "/HH/HHHistStor.txt", sep=""), header=T)
	HHFlood <<- read.table(paste(input_dir, "/HH/HHFlood.txt", sep=""), header=T)
	JanToAprFlood <<- read.table(paste(input_dir, "/HH/HHFlood_JanApr.txt", sep=""), header=T)
	HHFlood_2_input <<- read.table(paste(input_dir, "/HH/HHFlood_2.txt", sep=""), header=T)
	APR_HH_input <<- read.table(paste(input_dir, "/HH/AprHH.txt", sep=""), header=T)
  
	### Kerr #####
	KerrFloodC <<- read.table(paste(input_dir, "/Kerr/FloodCurve.txt", sep=""), header=T)
	Article_56_input <<- read.table(paste(input_dir, "/Kerr/article_56.txt", sep=""), header=T)
	...
	AFFlood <<- read.table(paste(input_dir, "/AFECCandFC/AFFC.txt", sep=""), header=T)
  
	### Grand Coulee #######
	GCFlood1_input <<- read.table(paste(input_dir, "GC/GCFlood1.txt", sep=""), header=T)
	...
	GCCriticalCurve_input <<- read.table(paste(input_dir, "GC/GCCriticalCurve.txt", sep=""), header=T)
	...
	##### Duncan ##########
	DUCriticalCurve_input <<- read.table(paste(input_dir, "DUECCandFC/DUCriticalCurve.txt", sep=""), header=T)
	...
	DUF_2_input <<- read.table(paste(input_dir, "DUECCandFC/DUFlood_2.txt", sep=""), header=T)
	...
	ARFlood_2_input <<- read.table(paste(input_dir, "ARECCandFC/ARFlood_2.txt", sep=""), header=T)
	ARCriticalCurve_input <<- read.table(paste(input_dir, "ARECCandFC/ARCriticalCurve.txt", sep=""), header=T)
	...
	MIFlood_712 <<- read.table(paste(input_dir, "MicaECCandFC/MIFlood7-12.txt", sep=""), header=T)
	...
	MICriticalCurve_input <<- read.table(paste(input_dir, "MicaECCandFC/MICriticalCurve.txt", sep=""), header=T)
