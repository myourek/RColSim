...
list_scenarios = c("Historical_baseline/baseline")
...
for (scr_no in 1:length(list_scenarios)) {
	...
	Supply_Input = read.table(paste("~/Step_3/Aggregate_demand_ColSim/to_step_4/GCAM/", gcm, "/supply_", scr, ".txt", sep=""), header=T)
	N = length(Supply_Input[,1])
	if (simulate_demand == 1) {
		Demand_Input = read.table(paste("~/Step_3/Aggregate_demand_ColSim/to_step_4/GCAM/", gcm, "/demand_", scr, ".txt", sep=""), header=T)
	} else {
	...
	if (simulate_demand == 1) {
		interruptible_demand = read.table(paste("~/Step_3/Aggregate_demand_ColSim/to_step_4/GCAM/", gcm, "/interruptible_", scr, ".txt", sep=""), header=T)
	} else {
...
############################# Assured Refill
	...
	run_type = ifelse(simulate_demand==1, "GCAM", "Supply_only")
