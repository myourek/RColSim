...
LBECC <- function() {
	if (month_in_year <= 5 || month_in_year == 12) {
		out1 <- max(LBCriticalCurve(), LBRefillCurve())
	} else {
...
HHCurFC_Cont <- function() {
	...
	if (month_in_year %in% 3:9) { # Oct - April flood evacuation, May - June refill
...
HHBiOpDraftLimit <- function() {
	if (month_in_year >= 6 && month_in_year <= 8) { ## Biological Opinion for Jan -- Mar
...
AFNetHead <- function() {
	AFTailElev <- 2042.2 # Tailwater elevation (ft)
	AFLoss <- 0
...
BRCurFC_Cont115 <- function() {
	...
	if (BR_FloodM_Range[length(BR_FloodM_Range)] < BRRunoffAprJuly) {
		...
		row_no <- min(2, max(1.0, which(BR_FloodM_Range > BRRunoffAprJuly)[1] - 1))
	}
	if (BRRunoffAprJuly <= 6.0E06) {
		if (month_in_year==6) {
			BRCurFC_Cont115_o <- 1170000
		} else if (month_in_year==7) {
			BRCurFC_Cont115_o <- BRForecastFloodStorage[row_no, 23] # FEB_BR115
		} else if (month_in_year==8) {
			BRCurFC_Cont115_o <- BRForecastFloodStorage[row_no, 24] # MAR_BR115
		} else if (month_in_year==9) {
			BRCurFC_Cont115_o <- BRForecastFloodStorage[row_no, 25] # APR_BR115
		} else {
...
# Brownlee does not have a critical curve, so the alternative uses medium year flood curve for May
# and BRTopVol for rest of the refill and fixed periods.
...
DWCurFC_Cont <- function() {
	...
	if (month_in_year %in% 3:11) { # October - June flood evacuation period
		if (DWRunoffAprJuly <= 3.8E06) {
			if (month_in_year %in% 3:5) {
				DWCurFC_Cont_o <- 2767950
			} else if (month_in_year == 6) {
				DWCurFC_Cont_o <- DW_FloodM$JAN_DW[row_no]
			} else if (month_in_year == 7) {
				DWCurFC_Cont_o <- DW_FloodM$FEB_DW[row_no]
			} else if (month_in_year == 8) {
				DWCurFC_Cont_o <- DW_FloodM$MAR_DW[row_no]
			} else if (month_in_year == 9) {
