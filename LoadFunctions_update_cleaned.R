...
MIMinRefill <- function() {
	...
	} else if (gurantee_refill == 2 || gurantee_refill == 3){
...
MIMinReq <- function() {
	...
	MIMinReq_1 <- min(max(Mica() + MIIn() - max(MIMinRefill(), MICriticalCurveMin()), MIAvgMin * cfsTOafw), MIAssuredRelease() * cfsTOafw)
	MIMinReq_2 <- min(max(Mica() + MIIn() - max(MIMinRefill(), MICriticalCurve()), MIAvgMin * cfsTOafw), MITargetRelease() * cfsTOafw)
...
MI_AprilOperatingCurve <- function() {
	MI_AprilOperatingCurve_0 <- MIOperatingRuleCurve_Apr
	return(MI_AprilOperatingCurve_0)
}
MI_JanOperatingCurve <- function() {
	MI_AprilOperatingCurve_0 <- MIOperatingRuleCurve_Jan
	return(MI_AprilOperatingCurve_0)
}
...
MICriticalCurveMin <- function() {
	if (CriticalCurveSw == 1) {
		...
		MICriticalCurveMin_o <- MICriticalCurve_input$CRC2[week_in_year]
	} else if (CriticalCurveSw == 3) {
		MICriticalCurveMin_o <- MICriticalCurve_input$CRC3[week_in_year]
	} else if (CriticalCurveSw == 4) {
		MICriticalCurveMin_o <- MICriticalCurve_input$CRC4[week_in_year]
	}
...
MIEnergySup <- function() { # Water released to satisfy firm and non-firm power requirements (AF/wk)
	...
	if (is.na(water_df$TotalEnergySup[week_counter])) {
		water_df$TotalEnergySup[week_counter] <<- TotalEnergySup()
	}
...
MIFloodRelSharedWater <- function() {
	MIFloodRelSharedWater_o <- max(0, Mica() + MIIn() - MIPrelim() - MICombSup() - MIECC())
	return(MIFloodRelSharedWater_o)
}
...
MIFloodRelFrac <- function() {
	if (TotalFloodRelSharedWater_c == 0) {
		MIFloodRelFrac_o <- 0
	} else {
		MIFloodRelFrac_o <- MIFloodRelSharedWater() / TotalFloodRelSharedWater_c
	}
	return(MIFloodRelFrac_o)
}
...
MIMinFloodRelReq <- function() { # Minimum release during refill period to ensure the reservoir does not refill too quickly, based on initial controlled flow
	MIMinFloodRelReq_o <- min(MIFloodRelSharedWater(), MinFloodRelReq() * MIFloodRelFrac())
	return(MIMinFloodRelReq_o)
}
MIDamProtectExcess <- function() {
	MIDamProtectExcess_o <- max(0, MIDamProtectRel() - MIPrelim() - MICombSup() + MIRelReducReq())
...
MIRelease <- function() { # Water released from Mica after accounting for all objectives
	#MIRelease_o <- max(min(max(MIMinReq(), MIPrelim() + MICombSup() - MIRelReducReq() + MIMinFloodRelReq()), MIRelLimit()), MIDamProtectRel())
	MIRelease_o <- max(min(MIPrelim() + MICombSup() - MIRelReducReq(), MIRelLimit()), MIDamProtectRel())
	return(MIRelease_o)
}
...
ARMinReq <- function() {
	...
	ARMinReq_o <- min(max(Arrow() + ARIn() - max(ARMinRefill(), ARCriticalCurveMin()), ARAvgMin * cfsTOafw), ARAssuredRelease() * cfsTOafw)
	return(ARMinReq_o)
}
...
AR_AprilOperatingCurve <- function() {
	AR_AprilOperatingCurve_0 <- AROperatingRuleCurve_Apr
	return(AR_AprilOperatingCurve_0)
}
AR_JanOperatingCurve <- function() {
	AR_JanOperatingCurve_0 <- AROperatingRuleCurve_Jan
	return(AR_JanOperatingCurve_0)
}
ARRuleReq <- function() {
...
ARECC <- function() {
	ARECC_o <- min(max(min(max(ARAssuredRefill(), ARCriticalCurve()), ARVariableRefill()), ARLowerLimit(), ARMinRefill()), ARFloodCurve())
	return(ARECC_o)
}
...
ARSharedWater <- function() {
	ARSharedWater_o <- max(Arrow() + ARIn() - ARPrelim() - ARMcNarySup() - max(ARMinRefill(), ARCriticalCurveMin()), 0)
...
ARCombUpProtect <- function() {
	...
	outflow <- ARPrelim() + ARCombSup_c - ARRelReducReq() + ARMinFloodRelReq()
...
ARDamProtectExcess <- function() {
	ARDamProtectExcess_o <- max(0, ARDamProtectRel() - ARPrelim() - ARCombSup_c + ARRelReducReq_c - 
		ARMinFloodRelReq_c - ARCombUpProtect_c)
...
ARRelease <- function() { 
	ARRelease_o <- max(min(ARPrelim() + ARCombSup_c - ARRelReducReq() + ARMinFloodRelReq() + ARCombUpProtect(), ARRelLimit()), ARDamProtectRel())
	return(ARRelease_o)
}
AROutflow <- function() {
...
DU_AprilOperatingCurve <- function() {
	DU_AprilOperatingCurve_0 <- DUOperatingRuleCurve_Apr
	return(DU_AprilOperatingCurve_0)
}
DU_JanOperatingCurve <- function() {
	DU_JanOperatingCurve_0 <- DUOperatingRuleCurve_Jan
	return(DU_JanOperatingCurve_0)
}
DURuleReq <- function() {
...
LB_AprilOperatingCurve <- function() {
	LB_AprilOperatingCurve_0 <- LBOperatingRuleCurve_Apr
	return(LB_AprilOperatingCurve_0)
}
LB_JanOperatingCurve <- function() {
	LB_JanOperatingCurve_0 <- LBOperatingRuleCurve_Jan
	return(LB_JanOperatingCurve_0)
}
LBRuleReq <- function() {
...
LBFloodRelSharedWater <- function() {
	LBFloodRelSharedWater_o <- max(0, Libby() + LBIn() - LBPrelim() - LBCombSup_c - LBECC())
	return(LBFloodRelSharedWater_o)
}
...
LBFloodRelFrac <- function() {
	if (TotalFloodRelSharedWater_c == 0) {
		LBFloodRelFrac_o <- 0
	} else {
		LBFloodRelFrac_o <- LBFloodRelSharedWater() / TotalFloodRelSharedWater_c
	}
	return(LBFloodRelFrac_o)
}
LBRelReducReq <- function() {
	LBRelReducReq_o <- TotalRelReducReq_c * LBFloodFrac()
	return(LBRelReducReq_o)
}
LBMinFloodRelReq <- function() { # Minimum release during refill period to ensure the reservoir does not refill too quickly, based on initial controlled flow
	LBMinFloodRelReq_o <- min(LBFloodRelSharedWater(), MinFloodRelReq_c * LBFloodRelFrac())
	return(LBMinFloodRelReq_o)
}
LBDamProtectExcess <- function() {
	LBDamProtectExcess_o <- max(0, LBDamProtectRel() - LBPrelim() - LBCombSup_c + LBRelReducReq())
...
LBRelease <- function() {
	#LBRelease_o <- max(min(LBPrelim() + LBCombSup() - LBRelReducReq() + LBMinFloodRelReq(), LBRelLimit()), LBDamProtectRel())
	LBRelease_o <- max(min(LBPrelim() + LBCombSup_c - LBRelReducReq(), LBRelLimit()), LBDamProtectRel())
	return(LBRelease_o)
}
LBOutflow <- function() {
...
CL_AprilOperatingCurve <- function() {
	CL_AprilOperatingCurve_0 <- CLOperatingRuleCurve_Apr
	return(CL_AprilOperatingCurve_0)
}
CL_JanOperatingCurve <- function() {
	CL_JanOperatingCurve_0 <- CLOperatingRuleCurve_Jan
	return(CL_JanOperatingCurve_0)
}
CLRuleReq <- function() {
...
HH_AprilOperatingCurve <- function() {
	HH_AprilOperatingCurve_0 <- HHOperatingRuleCurve_Apr
	return(HH_AprilOperatingCurve_0)
}
HH_JanOperatingCurve <- function() {
	HH_JanOperatingCurve_0 <- HHOperatingRuleCurve_Jan
	return(HH_JanOperatingCurve_0)
}
HHRuleReq <- function() {
...
HHFloodRelSharedWater <- function() {
	HHFloodRelSharedWater_o <- max(0, HungryHorse() + HHIn() - HHPrelim() - HHCombSup_c - HHECC())
	return(HHFloodRelSharedWater_o)
}
...
HHFloodRelFrac <- function() {
	if (TotalFloodRelSharedWater_c == 0) {
		HHFloodRelFrac_o <- 0
	} else {
		HHFloodRelFrac_o <- HHFloodRelSharedWater() / TotalFloodRelSharedWater_c
	}
	return(HHFloodRelFrac_o)
}
HHRelReducReq <- function() {
	HHRelReducReq_o <- TotalRelReducReq_c * HHFloodFrac()
	return(HHRelReducReq_o)
}
HHMinFloodRelReq <- function() { 
	HHMinFloodRelReq_o <- min(HHFloodRelSharedWater(), MinFloodRelReq_c * HHFloodFrac())
	return(HHMinFloodRelReq_o)
}
HHDamProtectExcess <- function() {
	HHDamProtectExcess_o <- max(0, HHDamProtectRel() - HHPrelim() - HHCombSup_c + HHRelReducReq())
...
HHRelease <- function() {
	#HHRelease_o <- max(min(HHPrelim() + HHCombSup() - HHRelReducReq() + HHMinFloodRelReq(), HHRelLimit()), HHDamProtectRel())
	HHRelease_o <- max(min(HHPrelim() + HHCombSup_c - HHRelReducReq(), HHRelLimit()), HHDamProtectRel())
	return(HHRelease_o)
}
HHOutflow <- function() {
...
KE_AprilOperatingCurve <- function() {
	KE_AprilOperatingCurve_0 <- KEOperatingRuleCurve_Apr
	return(KE_AprilOperatingCurve_0)
}
KE_JanOperatingCurve <- function() {
	KE_JanOperatingCurve_0 <- KEOperatingRuleCurve_Jan
	return(KE_JanOperatingCurve_0)
}
KETopVol <- function() {
	if (KerrTopVolSw == 0) {
...
KEDamProtectExcess <- function() {
	KEDamProtectExcess_o <- max(0, KEDamProtectRel() - KEPrelim() - KECombSup_c - KECombUpProtect_c)
...
KERelease <- function() {
	KERelease_o <- max(min(KEPrelim() + KECombSup_c + KECombUpProtect(), KERelLimit()), KEDamProtectRel())
	return(KERelease_o)
}
KEOutflow <- function() {  
...
AFDamProtectRel <- function() {
	AFDamProtectRel_o <- max(AlbeniFalls() + AFInflow() - AFFullPoolVol, 0)
...
AF_AprilECC <- function() {
	AF_AprilECC_o <- min(max(AFCriticalCurve_input[36,2], AFAssuredRefill_input[36,2]), AF_AprilFloodCurve)
	return(AF_AprilECC_o)
}
AF_JanECC <- function() {
	AF_JanECC_o <- min(max(AFCriticalCurve_input[23,2], AFAssuredRefill_input[23,2]), AF_JanFloodCurve)
	return(AF_JanECC_o)
}
AF_AprilOperatingCurve <- function() {
	AF_AprilOperatingCurve_0 <- AFOperatingRuleCurve_Apr
	return(AF_AprilOperatingCurve_0)
}
AF_JanOperatingCurve <- function() {
	AF_JanOperatingCurve_0 <- AFOperatingRuleCurve_Jan
	return(AF_JanOperatingCurve_0)
}
AFRuleReq <- function() {
...
AFCombUpProtect <- function() {
	inflow <- AFIn() + AFCombUpSup() + KEDamProtectExcess()
...
#	AprilUpstreamFloodEvacGC_o <- (AF_JanECC() - AF_AprilECC()) + (AR_JanECC() - AR_JanECC()) + 
...
#	AprilUpstreamFloodEvacGC_o <- (AF_JanFloodCurve() - AF_AprilFloodCurve()) + (AR_JanFloodCurve() - AR_AprilFloodCurve()) + 
#	(CL_JanFloodCurve() - CL_AprilFloodCurve()) + (DU_JanFloodCurve() - DU_AprilFloodCurve()) + (HH_JanFloodCurve() - HH_AprilFloodCurve()) + 
#	(KE_JanFloodCurve() - KE_AprilFloodCurve()) + (LB_JanFloodCurve() - LB_AprilFloodCurve()) + (MI_JanFloodCurve() - MI_AprilFloodCurve())
#	return(AprilUpstreamFloodEvacGC_o)
#}
AprilUpstreamFloodEvacGC <- function() {
	AprilUpstreamFloodEvacGC_o <- (AF_JanOperatingCurve() - AF_AprilOperatingCurve()) + (AR_JanOperatingCurve() - AR_AprilOperatingCurve()) + 
	(CL_JanOperatingCurve() - CL_AprilOperatingCurve()) + (DU_JanOperatingCurve() - DU_AprilOperatingCurve()) + (HH_JanOperatingCurve() - HH_AprilOperatingCurve()) + 
	(KE_JanOperatingCurve() - KE_AprilOperatingCurve()) + (LB_JanOperatingCurve() - LB_AprilOperatingCurve()) + (MI_JanOperatingCurve() - MI_AprilOperatingCurve())
	return(AprilUpstreamFloodEvacGC_o)
}
...
GC_April_Target <- function() {
	...
	GCAssuredRefill_o <- GCAssuredRefill_input$GrandCoulee[36]
	GCCriticalCurve_o <- GCCriticalCurve_input$CRC1[36]
	#GC_April_Target_o <- min(max(GCAssuredRefill_o, GCCriticalCurve_o), GC_AprilFC_o)
...
#		UpstreamFloodEvacGC_o <- (AF_JanFloodCurve() - AFFloodCurve()) + (AR_JanFloodCurve() - ARFloodCurve()) + 	
#		(CL_JanFloodCurve() - CLFloodCurve()) + (DU_JanFloodCurve() - DUFloodCurve()) + (HH_JanFloodCurve() - HHFloodCurve()) + 
#		(KE_JanFloodCurve() - KEFloodCurve()) + (LB_JanFloodCurve() - LBFloodCurve()) + (MI_JanFloodCurve() - MIFloodCurve())
#	} else {
...
UpstreamFloodEvacGC <- function() {
	if (week_in_year %in% 23:36) {
		UpstreamFloodEvacGC_o <- (AF_JanOperatingCurve() - AFOperatingRuleCurve) + (AR_JanOperatingCurve() - AROperatingRuleCurve) + 	
		(CL_JanOperatingCurve() - CLOperatingRuleCurve) + (DU_JanOperatingCurve() - DUOperatingRuleCurve) + (HH_JanOperatingCurve() - HHOperatingRuleCurve) + 
		(KE_JanOperatingCurve() - KEOperatingRuleCurve) + (LB_JanOperatingCurve() - LBOperatingRuleCurve) + (MI_JanOperatingCurve() - MIOperatingRuleCurve)
	} else {
		UpstreamFloodEvacGC_o <- 0
	}
	water_df$UpstreamFloodEvacGC[week_counter] <<- UpstreamFloodEvacGC_o
...
GCECC <- function() {
	...
	#GCECC_o <- min(max(GCAssuredRefill(), GCCriticalCurve(), GCLowerLimit(), GCRecLimit(), GCFerryLimit()), GCSummerDraftLimit(), GC_VDL())
	return(GCECC_o)
}
...
GCBONAvailWater <- function() {
	if (fish_over_refill == 1) {
		GCBONAvailWater_o <- max(0, GrandCoulee() + GCIn_c - GCPrelim() - GCAssuredRefill())
	} else if (fish_over_refill == 0) {	
		if (week_in_year < 23) {
			GCBONAvailWater_o <- max(0, GrandCoulee() + GCIn_c - GCPrelim() - 8.2e6)
		} else {
			GCBONAvailWater_o <- max(0, GrandCoulee() + GCIn_c - GCPrelim() - GCECC())
		}
	}
	return(GCBONAvailWater_o)
}
...
GCSupForBON <- function() {
	GCSupForBON_o <- max(min(GCBONAvailWater(), BONFlowDeficit()), 0)
	return(GCSupForBON_o)
}
GCFishSup <- function() {
	GCFishSup_o <- max(GCSupForBON(), GCSupForVernitaBar(), GCMcNarySup())
	return(GCFishSup_o)
}
...
GCSharedWater <- function() {
	GCSharedWater_o <- max(0, GrandCoulee() + GCIn_c - GCPrelim() - GCFishSup() - max(GCMinRefill(), GCCriticalCurveMin()))
...
GCCombUpSup <- function() {
	...
	if (fish_over_refill == 1) {
		...
		if (week_in_year %in% c(1:52)) {
			GCCombUpSup_o <- min(max(0, GrandCoulee() + GCIn_c + GCCombUpSup_1 - GCPrelim() - GCECC()), GCCombUpSup_1)
		} else {
			GCCombUpSup_o <- GCCombUpSup_1
		}
	}
...
GCMinFloodRelReq <- function() { # Minimum release during refill period to ensure the reservoir does not refill too quickly, based on initial controlled flow
	...
	GCMinFloodRelReq_o <- min(GCFloodRelSharedWater(), MinFloodRelReq_c)
	water_df$GCMinFloodRelReq[week_counter] <<- GCMinFloodRelReq_o
	return(GCMinFloodRelReq_o)
}
GCCombUpProtect <- function() {
	inflow <- GCIn_c + GCCombUpSup() + AFDamProtectExcess() + ARDamProtectExcess() + CLDamProtectExcess()
	outflow <- GCPrelim() + GCCombSup_c - TotalRelReducReq_c + GCMinFloodRelReq()
	GCCombUpProtect_o <- max(min(GrandCoulee() + inflow - outflow - GCECC(), AFDamProtectExcess() + ARDamProtectExcess() + CLDamProtectExcess()), 0)
	return(GCCombUpProtect_o)
}
GCDamProtectExcess <- function() {
	GCDamProtectExcess_o <- max(0, GCDamProtectRel() - GCPrelim() - GCCombSup_c + TotalRelReducReq_c - GCMinFloodRelReq() - GCCombUpProtect())
...
GCRelease <- function() {
	GCRelease_o <- max(min(GCPrelim() + GCCombSup_c - TotalRelReducReq_c + GCMinFloodRelReq() + GCCombUpProtect(), GCRelLimit()), GCDamProtectRel())
	return(GCRelease_o)
}
GCOutflow <- function() {
...
PALDamProtectExcess <- function() {
	PALDamProtectExcess_o <- max(0, PALDamProtectRel() - PALPrelim() - PALCombUpProtect_c)
...
BRCriticalCurveMin <- function() {
	BRCriticalCurve_o <- BRCriticalCurve_input$CRC4[week_in_year] # year 4 critical curve
...
BRCombUpProtect <- function() {
	excess <- PayetteDamProtectExcess() + BoiseDamProtectExcess() + OWYDamProtectExcess() + MINDamProtectExcess()
	inflow <- BRIn_c + excess
	outflow <- BRPrelim() + BRCombSup()
...
BRDamProtectExcess <- function() {
	BRDamProtectExcess_o <- max(0, BRDamProtectRel() - BRPrelim() - BRCombSup_c - BRCombUpProtect_c)
...
DWCriticalCurveMin <- function() {
	DWCriticalCurve_o <- DWCriticalCurve_input$CRC4[week_in_year] # year 4 critical curve
...
TotalEnergySup <- function() {
	TotalEnergySup_o <- AREnergySup() + DUEnergySup() + LBEnergySup() + CLEnergySup() + HHEnergySup() + KerrEnergySup() +
		AFEnergySup() + GCEnergySup() + BREnergySup() + DWEnergySup() 
	return(TotalEnergySup_o)
}	
...
TotalFloodRelSharedWater <- function() {
	#TotalFloodRelSharedWater_o <- MIFloodRelSharedWater() + ARFloodRelSharedWater() + GCFloodRelSharedWater() + LBFloodRelSharedWater() + HHFloodRelSharedWater()
...
MinFloodRelReq <- function() {
	if (is.na(start_refill_wk)) {
		...
		MinFloodRelReq_o <- max(ControlledFlow() * cfsTOafw - DAPrelim() - TotalFishSup() - TotalEnergySup(),0)
	} else {
		MinFloodRelReq_o <- 0
	}
...
TotalDamProtectExcess <- function() {
	TotalDamProtectExcess_o <- GCDamProtectExcess() + BRDamProtectExcess() + DWDamProtectExcess()
...
AvgNonFirmLoad <- function() {
	...
	AvgNonFirmLoad_o <- 1.98e6 - AvgFirmLoad()
