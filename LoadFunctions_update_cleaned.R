...
ARFloodRelSharedWater <- function() {
	ARFloodRelSharedWater_o <- max(0, Arrow() + ARIn() - ARPrelim() - ARCombSup() - ARECC())
...
ARRelease <- function() { 
	ARRelease_o <- max(min(max(ARMinReq(), ARPrelim() + ARCombSup() - ARRelReducReq() + ARMinFloodRelReq()), ARRelLimit()), ARDamProtectRel())
	return(ARRelease_o)
}
AROutflow <- function() {
...
LBFloodRelSharedWater <- function() {
	LBFloodRelSharedWater_o <- max(0, Libby() + LBIn() - LBPrelim() - LBCombSup() - LBECC())
...
LBMinFloodRelReq <- function() { # Minimum release during refill period to ensure the reservoir does not refill too quickly, based on initial controlled flow
	LBMinFloodRelReq_o <- min(LBFloodRelSharedWater(), MinFloodRelReq() * LBFloodRelFrac())
...
HHFloodRelSharedWater <- function() {
	HHFloodRelSharedWater_o <- max(0, HungryHorse() + HHIn() - HHPrelim() - HHCombSup() - HHECC())
...
HHMinFloodRelReq <- function() { 
	HHMinFloodRelReq_o <- min(HHFloodRelSharedWater(), MinFloodRelReq() * HHFloodFrac())
...
AF_April_ECC <- function() {
	AF_April_AssuredRefill <- AFAssuredRefill_input[36, 2]
	AF_April_CriticalCurve <- AFCriticalCurve_input$CRC1[36]
	AF_April_FloodCurve <- AFFlood_input$Max[36]
	AF_April_ECC_o <- min(max(AF_April_AssuredRefill, AF_April_CriticalCurve, AF_April_FloodCurve))
	return(AF_April_ECC_o)
}
...
#	UpstreamFloodEvacGC_1 <- (AFRelease_c - AFInflow()) + (ARRelease_c - ARInflow()) + (CLRelease_c - CLInflow()) + (DURelease_c - DUInflow()) +
#		(HHRelease_c - HHInflow()) + (KERelease_c - KEInflow()) +(LBRelease_c - LBInflow()) + (MIRelease_c - MIInflow())
#	UpstreamFloodEvacGC_2 <- water_df$UpstreamFloodEvacGC[week_counter-1]
#	if (week_in_year == 23) {
#		UpstreamFloodEvacGC_c <<- 0
#	} else {
#		UpstreamFloodEvacGC_c <<- UpstreamFloodEvacGC_1 + UpstreamFloodEvacGC_2
#	}
#	water_df$UpstreamFloodEvacGC[week_counter] <<- UpstreamFloodEvacGC_c
#	return(UpstreamFloodEvacGC_c)
#}
UpstreamFloodEvacGC <- function() {
...
GCVariableDraftLimit <- function() {
	if (week_in_year>=23 && week_in_year<=36) {
		in_o <- PRResidualInflowJanMar + (AprilUpstreamFloodEvacGC() - UpstreamFloodEvacGC())
		out_o <-  GCBdgtForVB()
...
GCECC <- function() {
	GCECC_o <- min(max(min(max(GCAssuredRefill(), GCCriticalCurve()), GCVariableRefill()), GCLowerLimit(), GCMinRefill(), GCRecLimit(), GCFerryLimit()), GCSummerDraftLimit(), GC_VDL())
...
GCSharedWater <- function() {
	GCSharedWater_o <- max(0, GrandCoulee() + GCIn_c - GCPrelim() - max(GCMcNarySup(), GCSupForVernitaBar()) - max(GCMinRefill(), GCCriticalCurveMin()))
...
GCMcNaryBONSup <- function() {
	GCMcNaryBONSup_o <- max(GCMcNarySup(), GCSupForVernitaBar())
	return(GCMcNaryBONSup_o)
}
GCCombSup <- function() {
	GCCombSup_o <- GCMcNaryBONSup() + GCCombUpSup() + GCEnergySup()
...
GCMcNaryBONSupEnergy <- function() { # Energy produced by releasing water to meet fish targets at McNary, Vernita Bar, and Bonneville
	GCMcNaryBONSupEnergy_o <- GCMcNaryBONSup() * TotalGCHead() * Estimated_Efficiency * MWhr_per_ftAcFt
...
GCFloodRelSharedWater <- function() {
	GCFloodRelSharedWater_o <- max(0, GrandCoulee() + GCIn() - GCPrelim() - GCCombSup_c - GCECC())
...
GCRelease <- function() {
	GCRelease_o <- max(min(max(min(BONFlowDeficit(), GCLimitedStorage()), GCMinReq(), GCPrelim() + GCCombSup_c - TotalRelReducReq() + GCMinFloodRelReq()), GCRelLimit()), GCDamProtectRel())
	return(GCRelease_o)
}
GCOutflow <- function() {
...
Cat4FilledStorPerc <- function() {
	if (control_wk == 9999) {
		...
		storage_o <- 100 * max(0, 1.0 - (((GCFullPoolVol - GrandCoulee()) + (ARFullPoolVol - Arrow())) / 
			((GCFullPoolVol - reservoir_vol_df$GCOUL[control_wk-1]) + (ARFullPoolVol - reservoir_vol_df$ARROW[control_wk-1]))))
	}
	return(storage_o)
}
ICF_adj <- function() {
